"""POLARIS Shield — cross-implementation interoperability corpus (PSTV) + an
...
the reference ``polaris_shield`` produces.
...
* ``altcodec``         — an INDEPENDENT PLSH decoder built from FORMAT.md alone
  (no reuse of ``polaris_shield``'s envelope/combiner/KDF code). It reuses only the
