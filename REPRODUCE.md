...
## 1. Tests (508) — functional + KAT + ACVP conformance + property/fuzz + interop

```bash
pip install -r requirements.txt
python -m pytest -q                       # expect: 508 passed
```
...
508-test suite**, exiting non-zero on any failure:

```bash
docker build -t vorlath-shield-verify -f Dockerfile .
docker run --rm vorlath-shield-verify        # Verifpal + Tamarin + 508 tests; non-zero on any failure
```
