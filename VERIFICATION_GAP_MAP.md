...
- The **tests** (508-suite, ACVP, interop, **cross-impl differential**, dudect) exercise the **code**.
...
| **Tamarin scope** (`shield.spthy`) | authenticated mode only. **Forward secrecy w.r.t. the SENDER's long-term key is now an unbounded lemma** (`forward_secrecy_under_sender_key_reveal`); FS w.r.t. the RECIPIENT's static key is OUT OF SCOPE BY DESIGN (one-pass static-recipient-key PKE). **Anonymous-mode flag-stripping** is not lifted into a distinct Tamarin rule | carried by the Verifpal models + the implementation/interop tests | "argued, not proved" only for anonymous-mode flag-stripping |
