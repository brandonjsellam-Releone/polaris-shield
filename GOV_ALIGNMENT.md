...
| Stateful hash signatures (XMSS/LMS) approved ONLY for low-volume firmware/software signing, strict state management, non-exporting hardware key-gen | SP 800-208 (2020) | partial (by design) | POLARIS deliberately chose STATELESS SLH-DSA over stateful XMSS/LMS. Upgrade: document this choice with the SP 800-208 constraints quoted, turning it into a standards-grounded justification. |
...
| Sovereign fault-tolerant quantum-compute trajectory (public anchor) | DARPA Quantum Benchmarking Initiative | out-of-scope | The BOREALIS citadel is speculative / order-of-magnitude and is honestly weaker than the public DARPA QBI program — a framing anchor, not a capability claim. |
